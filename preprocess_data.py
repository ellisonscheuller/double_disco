...
def clean_collection_vs_leptons(arrays, coll_prefix, radius):
    """
    Remove objects in coll_prefix within ΔR<radius of ANY lepton (Electrons + Muons vtx/no-vtx).
    ...
    # Leptons (no tight-ID here). arrays is an Awkward record, not a dict.
    ...
    # Replace all branches with this prefix using ak.with_field (no arrays.copy())
    out = arrays
...
for idx_file, fname in enumerate(files, 1):
    try:
        ...
        with uproot.open(fname)["Events"] as tree:
            ...
            # Stack: Electrons, Muons, Photons, Jets, FatJets, MET (MET last)
            events = arrays["event"]
            added = 0
